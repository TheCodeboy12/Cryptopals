...
def check_occurences(decrypted_text):
    '''checked how many times a charater occured and retuns a percrntage list tuples of them'''
...
'''
Objectives:
1. We need to adjust the code to check each hex in the hex list.
'''
def main():
    sets_of_decrypted_texts=[[single_byte_XOR_final(hexush,char) for char in charlist2] for hexush in hexushesh]
    #[check_occurences(list_of_decrypted_texts) for item in list_of_decrypted_texts]
    ##list_of_letter_scores=[check_occurences(item) for item in sets_of_decrypted_texts]
    sets_of_scores_by_letter=[[check_scores(dycrypted_text) for dycrypted_text in sets] for sets in list_of_decrypted_texts]
...
[[[dictionary_to_sorted_string(tupset) for tupset in set_of_decrypted_occurences] for tupset in set_of_decrypted_occurences] for set_of_decrypted_occurences in sets_of_scores_by_letter]

    print(f'The encrypted hex text is \n:{hexush}\n')
    print(f'Decrypted text is: \n{sets_of_decrypted_texts[numscores.index(max(numscores))]}')
